  ...
  return <CollapsibleSection controls="fuel" open={open} step={2} title="Fuel details" summary={summary} buttonRef={buttonRef} onOpen={onOpen}>
    <>
      ...
      <div id="fuel-content" className={classes('fuel-fields')}>
        ...
        <div className={classes('form-field')}><label htmlFor="fuel-type">Fuel type (optional)</label><input id="fuel-type" value={settings.fuelType ?? ''} placeholder="e.g. Petrol" onChange={(event) => updateSettings({ fuelType: event.target.value })} /></div>
      </div>
      {complete && <button className={classes('done-button')} type="button" onClick={onDone}>Done with fuel details <ArrowRight size={18} /></button>}
    </>
  </CollapsibleSection>
}
