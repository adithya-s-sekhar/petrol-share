...
export function ExpensesPanel({ draft, errors, stopsById, onChange, onAdd, onUpdate }: ExpensesPanelProps) {
  return <section className={layout('panel')} aria-labelledby="expenses-title">
    ...
    <div className={layout('expense-list')}>
      {(draft.expenses ?? []).map((expense, index) => {
        ...
        return <article className={layout('expense-row')} key={expense.id} aria-label={label}>
          ...
          <div className={layout('expense-scope')} role="radiogroup" aria-label={`${label} applies to`}>
