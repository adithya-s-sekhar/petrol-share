...
import { economyFromKmpl, economyToKmpl, formatCurrency, priceFromPerLitre, priceToPerLitre, unitLabels, type TripDraft, type UnitSystem } from '../../../domain'
...
interface FuelPanelProps {
  ...
  onCurrencyUse: (currency: string) => void
  onApplyVehiclePreset: (preset: VehiclePreset) => void
}

export function FuelPanel({ currencies, draft, errors, open, complete, unitSystem, units, recentCurrencies, vehiclePresets, buttonRef, onOpen, onDone, onUpdate, onCurrencyUse, onApplyVehiclePreset }: FuelPanelProps) {
  ...
  return <CollapsibleSection controls="fuel" open={open} step={2} title="Fuel details" summary={summary} buttonRef={buttonRef} onOpen={onOpen}>
    <>
      ...
      {vehiclePresets.length > 0 && <section className={layout('fuel-presets')} aria-labelledby="fuel-presets-title">
        <div><h3 id="fuel-presets-title">Saved vehicles</h3><p>Apply a preset here without leaving fuel details.</p></div>
        <div className={layout('fuel-preset-list')}>{vehiclePresets.map((preset) => {
          const presetUnits = unitLabels(preset.preferredUnits)
          const unitName = preset.preferredUnits === 'metric' ? 'Metric' : preset.preferredUnits === 'us' ? 'US customary' : 'UK imperial'
          return <div className={layout('fuel-preset')} key={preset.id}>
            <div><strong>{preset.name}</strong><span>{displayNumber(preset.fuelEconomyKmpl, (value) => economyFromKmpl(value, preset.preferredUnits))} {presetUnits.economy} · {unitName} · {preset.fuelType || 'No fuel type'}</span></div>
            <Button variant="secondary" onClick={() => onApplyVehiclePreset(preset)}>Apply</Button>
          </div>
        })}</div>
      </section>}
      <div id="fuel-content" className={layout('fuel-fields')}>
        ...
        <div className={layout('form-field currency-field')}><label htmlFor="currency">Currency</label><input id="currency" list="currency-options" role="combobox" autoComplete="off" value={settings.currency} aria-invalid={Boolean(errors['fuelSettings.currency'])} aria-describedby="currency-help currency-error" onBlur={() => onCurrencyUse(settings.currency)} onChange={(event) => updateSettings({ currency: event.target.value.toUpperCase() })} /><datalist id="currency-options">{currencies.map(({ code, symbol, name }) => <option key={code} value={code}>{symbol} · {code} · {name}</option>)}</datalist><small id="currency-help">Search by symbol, code, or currency name.</small>{recentCurrencies.length > 0 && <div className={layout('recent-currencies')} aria-label="Recently used currencies"><span>Recent</span>{recentCurrencies.map((code) => <button key={code} type="button" aria-pressed={settings.currency === code} onClick={() => { updateSettings({ currency: code }); onCurrencyUse(code) }}>{code}</button>)}</div>}<FieldError id="currency-error" message={errors['fuelSettings.currency']} /></div>
