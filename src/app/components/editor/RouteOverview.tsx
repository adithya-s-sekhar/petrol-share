...
export function RouteOverview({ draft, unitSystem, distanceUnit }: Props) {
  ...
  return (
    <section className={layout('route-overview')} aria-labelledby="route-overview-title">
      <div className={layout('route-overview-heading')}>
      ...
      <p className={layout('sr-only')}>{draft.stops.map((stop, index) => {
      ...
      <div className={layout('route-overview-list')} aria-hidden="true">
        {draft.stops.map((stop, index) => {
          ...
          return (
            <div key={stop.id}>
              ...
              {leg && (
                <div className={layout('route-overview-leg')}>
