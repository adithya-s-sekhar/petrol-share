  ...
  return <CollapsibleSection controls="route" open={open} step={1} title="Build your route" summary={summary} buttonRef={buttonRef} onOpen={props.onOpen}>
    <>
      ...
      <div id="route-content">
        <ol className={layout('stops-list')}>
          {draft.stops.map((stop, index) => {
            const errorId = `stop-${stop.id}-error`
            const error = errors[`stops.${index}.name`]
            return <li className={layout('stop-row')} key={stop.id}>
              <span className={layout('stop-index')} aria-hidden="true">{index + 1}</span>
              <div className={layout('field-grow')}><label className={layout('row-label')} htmlFor={`stop-${stop.id}`}>Stop {index + 1}</label><div className={layout('input-with-icon')}><MapPin size={18} /><input id={`stop-${stop.id}`} aria-label={`Stop ${index + 1} name`} value={stop.name} placeholder={index === 0 ? 'Starting point' : 'Next stop'} aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined} onChange={(event) => props.onChangeStops(draft.stops.map((item) => item.id === stop.id ? { ...item, name: event.target.value } : item))} /></div><FieldError id={errorId} message={error} /></div>
              <div className={layout('row-actions')}><IconButton label={`Move stop ${index + 1} up`} disabled={index === 0} onClick={() => props.onMoveStop(index, -1)}><ArrowUp /></IconButton><IconButton label={`Move stop ${index + 1} down`} disabled={index === draft.stops.length - 1} onClick={() => props.onMoveStop(index, 1)}><ArrowDown /></IconButton><IconButton label={`Remove stop ${index + 1}`} destructive disabled={draft.stops.length <= 2} onClick={() => props.onRemoveStop(stop.id, index)}><Trash2 /></IconButton></div>
            </li>
          })}
        </ol>
        <Button className="w-full" onClick={props.onAddStop}><Plus /> Add another stop</Button>
        {draft.stops.length > 1 && draft.stops.every(({ name }) => name.trim()) && draft.stops[0].name.trim().toLocaleLowerCase() !== draft.stops.at(-1)?.name.trim().toLocaleLowerCase() && <Button className="mt-2 w-full" onClick={props.onMakeRoundTrip}><RotateCcw /> Make round trip</Button>}
        {returnStops.length > 0 && <div className={layout('return-stops')} aria-label="Return to an earlier stop"><span>Going back?</span><div>{returnStops.map((stop) => <button key={stop.id} className={layout('return-stop-button')} type="button" onClick={() => props.onReturnToStop(stop)}><RotateCcw size={15} /> Return to {stop.name.trim()}</button>)}</div><p>The known distance is reused when available, and can still be changed.</p></div>}
        <RouteOverview draft={draft} unitSystem={unitSystem} distanceUnit={units.distance} />
        <div className={layout('subsection')}>
          <h3>Leg distances</h3>
          <div className={layout('unit-picker')} aria-label="Display units">{UNIT_SYSTEM_OPTIONS.map(([value, label]) => <button key={value} type="button" aria-pressed={unitSystem === value} onClick={() => props.onUnitSystemChange(value)}>{label}</button>)}</div>
          <div className={layout('leg-list')}>
            {draft.legs.map((leg, index) => {
              const error = errors[`legs.${index}.distanceKm`]
              const errorId = `leg-${leg.id}-error`
              return <div className={layout('leg-row')} key={leg.id}>
                <div className={layout('leg-name')}><span title={stopsById.get(leg.fromStopId)}>{stopsById.get(leg.fromStopId)}</span><ArrowRight size={16} /><span title={stopsById.get(leg.toStopId)}>{stopsById.get(leg.toStopId)}</span></div>
                <div><label className={layout('row-label')} htmlFor={`leg-${leg.id}`}>Distance ({units.distance})</label><div className={layout('unit-input')}><input id={`leg-${leg.id}`} aria-label={`Distance from ${stopsById.get(leg.fromStopId)} to ${stopsById.get(leg.toStopId)} in ${units.distanceLong}`} type="number" inputMode="decimal" min="0" step="any" placeholder="0" value={displayNumber(leg.distanceKm, (value) => distanceFromKm(value, unitSystem))} aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined} onBlur={() => props.onReuseReverseDistance(leg.id)} onChange={(event) => { const value = numberFromInput(event.target.value); props.onUpdate({ ...draft, legs: draft.legs.map((item) => item.id === leg.id ? { ...item, distanceKm: value === null ? null : distanceToKm(value, unitSystem), distanceSource: 'manual' } : item) }) }} /><span>{units.distance}</span></div>{leg.distanceKm !== null && <span className={layout(`distance-source distance-source-${leg.distanceSource ?? 'manual'}`)} title={leg.distanceSource === 'reused' ? 'Reused from the reverse leg' : undefined}>{leg.distanceSource === 'reused' ? 'Auto-filled' : leg.distanceSource === 'lookup' ? 'Looked up' : leg.distanceSource === 'copied' ? 'Copied' : 'Manual'}</span>}{index > 0 && <Button variant="quiet" className="mt-2 w-full whitespace-normal text-center" onClick={() => props.onCopyPreviousLeg(leg.id)}><Copy /> Copy previous distance</Button>}<Button variant="quiet" className="mt-2 w-full whitespace-normal text-center" onClick={() => props.onShowMapDialog(leg.id)}><Search /> Look up road distance</Button><FieldError id={errorId} message={error} /></div>
              </div>
            })}
          </div>
        </div>
        {complete && <Button className="mt-5 w-full" onClick={props.onDone}>Done with route <ArrowRight /></Button>}
      </div>
    </>
  </CollapsibleSection>
}
